    ...
    with c00:
        with st.container(border=True, height = 200):   
            with st.form("form01", border=False):
                # sort option so we can the used index in st.selectbox
                sel_box_options = list(ss['dapar']['npdata'].keys())
                sel_box_options.sort()
                # st.write(sel_box_options)
                npz_finame = st.selectbox("Select data with extracted features", options = sel_box_options, index = 6)
